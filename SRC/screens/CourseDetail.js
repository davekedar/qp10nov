...
class CourseDetail extends Component {
  ...
  loading = async () => {
    ...
    try {
      let { data } = await axios
        .post("https://www.qualpros.com/api/get_one_course", {
          course_id: course_id,
          student_id: userid
        })
        .then(response => {
          console.log(response.data.data.course_detail_array.course_id);
          if (response.data.data.status === "success") {
            this.setState({
              course_details: response.data.data.course_detail_array
            });
            this.setState({
              course_lession_details: response.data.data.course_chapter_array
            });
            this.setState({
              course_ratings_details: response.data.data.course_reviews
            });
            this.setState({
              course_chapter_array: response.data.data.course_chapter_array
            });
            this.setState({
              course_material_array: response.data.data.course_material_array
            });
          } else {
  ...
  onStarRatingPress = async rating => {
    ...
    try {
      let { data } = await axios
        .post("https://www.qualpros.com/api/post_course_rating", {
          course_id: course_id,
          student_id: userid,
        ...
        <ScrollView style={styles.container}>
          <Card key="1">
            <CardItem>
              <Left>
                <Thumbnail
                  source={{ uri: this.state.course_details.tutor_image_url }}
                />
                <Body>
                  <Text style={{ width: 220 }}>
                    {this.state.course_details.tutor_name}
                  </Text>
            ...
            <CardItem style={{ marginRight: 10 }}>
              <Left>
                <Image
                  source={{ uri: this.state.course_details.course_image }}
                  style={{ height: 120, width: 150 }}
                />
                <Body>
                  <Text
                    style={{ width: 220, fontSize: 17, fontWeight: "bold" }}
                  >
                    {this.state.course_details.course_title}
                  </Text>
                  ...
                  >
                    £{this.state.course_details.course_price}
                  </Text>
              ...
              <Left>
                <Body>
                  <Text
                    style={{ width: 220, fontSize: 17, fontWeight: "bold" }}
                  >
                    {this.state.course_details.course_duration}
                  </Text>
                </Body>
              </Left>
              <Right>
          ...
          <Card>
            <CardItem header bordered>
              <Text style={{ color: "#d91009" }}>Course Chapter</Text>
            </CardItem>
            {this.state.course_chapter_array.map(course_chapter => {
              return (
                <Content key={course_chapter.chapter_id}>
                  <List>
                    <ListItem avatar>
                      <Left />
                      <Body>
                        <TouchableOpacity
                          onPress={() => {
                            this.props.navigation.navigate("Chapter_detail", {
                              chapter_id: course_chapter.chapter_id,
                              course_id: this.state.course_details.course_id
                            });
                          }}
                        >
                          <Text>{course_chapter.chapter_title}</Text>
                        </TouchableOpacity>
              ...
              return (
                <Content key={course_material.material_id}>
                  <List>
                    <ListItem>
                      <Left>
                        <Text>{course_material.material_name}</Text>
                      </Left>
                ...
                <Content key={course_rate.review_id}>
                  <List>
                    <ListItem avatar>
                      <Left>
                        <Thumbnail
                          source={{ uri: course_rate.profile_picture }}
                        />
                      </Left>
                      <Body>
                        <Text>{course_rate.name}</Text>
                        <StarRating
                          style={{ width: 220 }}
                          disabled={false}
                          maxStars={5}
